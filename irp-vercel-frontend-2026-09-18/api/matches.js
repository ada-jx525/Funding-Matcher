...
return async function handler(request, response) {
  ...
  if (request.method !== "POST") {
    return proxy(request, response, "/api/matches", {
  ...
  if (!Number.isSafeInteger(topK) || topK < 1 || topK > MAX_TOP_K) {
    return send(response, 400, { error: `top_k must be an integer between 1 and ${MAX_TOP_K}.` });
  }
  ...
  request.body = { profile_id: body.profile_id.trim(), top_k: topK, search_text: searchText.trim() || null };
  return proxy(request, response, "/api/matches", {
    methods: ["POST"], principal
  });
  }, { authenticate });
